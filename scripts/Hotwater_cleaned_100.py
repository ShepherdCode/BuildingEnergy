...
# ## Hotwater_cleaned Data
# Dataset Assume user downloaded archive.zip from Kaggle, renamed the file BuildingData.zip, and stored the file in the data subdirectory. Assume the zip file contains the hotwater_cleaned.csv file.
...
HOTWATER_CLEANED_FILE='hotwater_cleaned.csv'
...
hotwater__cleaned_df = read_zip_to_panda(ZIP_PATH,HOTWATER_CLEANED_FILE)
hotwater__cleaned_df = fix_date_type(hotwater__cleaned_df)
print(hotwater__cleaned_df.info()) 


# In[17]:


cols=hotwater__cleaned_df.columns
    ...
    for site in uniq_sites:
        for bldg in cols:
            if bldg.startswith(site):
                temp_df = hotwater__cleaned_df[bldg]
